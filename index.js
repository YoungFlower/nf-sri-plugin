...
////////////////////////////////////////////////////////////////////////////////
const unReadFile = ['https://eu-library.klarnaservices.com/lib.js','https://x.klarnacdn.net/kp/lib/v1/api.js']
function sri() {
    let config;
    const bundle = {};
    return {
        ...
        async writeBundle(options, _bundle) {
            ...
            Object.entries(bundle).forEach(([,bundleItem])=>{
                console.log(bundleItem.fileName)
            })
            ...
            htmls.forEach(async ({ name, source: html }) => {
                ...
                const calculateIntegrityHashes = async (element) => {
                    ...
                    const resourcePath = resourceUrl.indexOf(config.base) === 0
                        ...
                        : resourceUrl;
                    if(!resourcePath.endsWith('.js') ||resourcePath.startsWith('http'))
                    return 
                    const t = Object.entries(bundle).find(([, bundleItem]) =>resourcePath.indexOf(bundleItem.fileName)>=0)?.[1];
                    if (!t) {
                    ...
                    if ((element.attribs.crossorigin === void 0 || element.attribs.crossorigin === '')) {
